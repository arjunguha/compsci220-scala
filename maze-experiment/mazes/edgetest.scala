...
// Assumes that the path ends on a loop around the g.start node
...
def printwalk(m: Map[Graph.Node,Graph.Node], n: Graph.Node): Unit =
    if (n == m(n)) {
        println(s"Start: $n")
    } else {
        println(n)
...
List[Graph.Node] = {
    ...
    val curr = util.minMember(dist, toVisit)
    if (curr == g.finish)
