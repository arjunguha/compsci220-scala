...
\begin{scalacode}
def insert[A](lessThan: (A, A) => Bool, x: A, lst: List[A]): List[A] = lst match {
  case Nil => List(x)
  case hd :: tl => {
    if (lessThan(x, hd)) {
      x :: hd :: tl
...
def sort[A](lessThan: (A, A) => Bool, lst: List[A]) = lst match {
  case Nil => Nil
...
\item
Write the \scalainline{zip} function, which pairs corresponding elements in a list:
...
\item
Write the \scalainline{groupBy} function, which groups the elements of
a list into sub-lists:

\begin{scalacode}
def groupBy[A](f: (A, A) => Bool, lst: List[A]): List[List[A]]

test("groupBy test") {
  def f(x: Int, y): Boolean = n % 3 == m % 3
  assert(groupBy(f, List(1, 2, 3, 4, 5, 6, 7, 8)) == 
...
We've seen functions that take other functions as arguments, such as \scalainline{map}:
...
\begin{itemize}
  ...
  But, what about the \scalainline{eval} function we wrote above, which consumes \scalainline{BinOp}s?
...
\newdiscussion{First-class and Higher-Order Functions -- Dan (Feb 3)}

\begin{instructor}
  \begin{itemize}
    \item Briefly introduce the first-class functions.
    \item Briefly introduce higher-order functions.
...
A \emph{higher-order function} consumes or returns other functions.  Note that \scalainline{compute} above takes a function (from \scalainline{Int} to \scalainline{Int}) and returns an Int, so by definition is is a higher-order function.

Together, these two concepts let you express complicated programs very simply.

\section{Exercise \#1}

You are in charge of a pat\'e factory.  One of the annoying facts about making pat\'e is that ducks and geese are friendly, so ducks are always mixing themselves in with the geese.  But you realize that you can make a little extra money for the company if you can build a machine that turns ducks into dog food.

Spec: Given a \scalainline{List[Bird]}, write a program that uses \scalainline{map} to output \scalainline{"pate"} whenever it encounters a \scalainline{Goose} and \scalainline{"dog food"} whenever it encounters a \scalainline{Duck}.

For your reference, here's an implementation for \scalainline{map}:
...
For example, given \scalainline{val birds = List(Duck, Duck, Goose)}, your program should produce \scalainline{List("dog food", "dog food", "pate")}.  As this is just an in-class exercise, you should not harm any actual ducks or geese.

\section{Exercise \#2}

Write a ScalaTest (consult your notes from Lecture \#1) that tests that your function really does convert \scalainline{List(Duck, Duck, Goose)} into \scalainline{List("dog food", "dog food", "pate")}.
...
Suppose I want to add all of the elements of \scalainline{List(1,2,3,4,5)}.  How might you do this with pencil and paper?  One way is something like:
...
There's a pattern here.  We started with an initial value (\scalainline{0}) and then we applied a function (\scalainline{+}) over and over again to our placeholder variable (\scalainline{sum}) and a new value from our list.  This is the pattern that \scalainline{fold} solves, but it is designed in such a way (using generic types \scalainline{A} and \scalainline{B}) so that you can use it in many situations, not just those involving addition.
...
Your boss likes your idea about turning ducks into dog food, but he wants to know first how much money you are likely to make in a variety of scenarios.  Pat\'e is fancy, and each can of pat\'e made from a Goose goes for \$10.  Each can of dog food made from a Duck goes for \$1.25.

Write a program that computes the amount of money you'll make given a \scalainline{List[Bird]}.

\section{Exercise \#4}

Write ScalaTests for the following inputs.  Make sure that all of the following cases produce the right sums (which you should compute by hand).

\scalainline{List(Duck, Duck, Goose)}
\scalainline{List(Goose, Goose, Goose, Goose, Duck, Goose)}
\scalainline{List()}
