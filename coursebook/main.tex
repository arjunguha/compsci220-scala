...
Describe the $A <: B$ notation.
Intuitively, the subtype may have more stuff, not not less.

Can implement several traits, such as comparable, ordered, etc.
...
def insert(x: Enumerable, alist: List[Enumerable]) = alist match {
  case Nil => List(x)
  case hd :: tl => {
    if (x.toInt <= hd.toInt) {
      x :: hd :: tl
...
def sort(alist: List[Enumerable]): List[Enumerable] = alist match {
  case Nil => Nil
  case hd :: tl => insert(hd, sort(tl))
}
\end{scalacode}

We lose track that we're working with a list of strings or whatever.

\begin{scalacode}
def insert[A <: Enumerable](x: A, alist: List[A]) = alist match {
  case Nil => List(x)
  case hd :: tl => {
    if (x.toInt <= hd.toInt) {
      x :: hd :: tl
...
def sort[A <: Enumerable](alist: List[A]): List[A] = alist match {
  case Nil => Nil
  case hd :: tl => insert(hd, sort(tl))
}
\end{scalacode}
...
\section{Lists as convariant containers}
...
Instead of using the silly toEnum function, let's implement a more normal
sorting function that uses comprable:

\begin{scalacode}
class C1 {
  def lessThan(other: C1) extends Comparable
}

class C2 {
  def lessThan(other: C2) extends Comparable
}

// Any does not work, because subtypes are more specific
trait Comparable {
  def lessThan(other: Any): Boolean
}

// Nothing requires the type of the receiver to be the same as T:
trait Comparable[T] {
  def lessThan(other: T): Boolean
}

class C1 extends Comparable[C1] {
  def lessThan(other: C1) extends Comparable
}

class C2 extends Comparable[C2] {
  def lessThan(other: C2) extends Comparable
}

def sort[T <: Comparable[T]](alist: List[T]): List[T]
\end{scalacode}

\newhw{Bounded Quantification Finger Exercises}

\newlecture

A hierarchy of animals, where each animal can only eat a particular kind of
food-type (as a type-member)
