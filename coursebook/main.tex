...
\section{Scheduling Exams}

One of the many responsibilities of the university registrar
is to come up with an exam schedule that works for all students.
The university has nearly 30,000 students who approximately 4 courses
from a menu of over 2,000 courses. During
exam period, each class has to be assigned a unique date, time, and
room, such that no student has to be in two places at the same
time. Fortunately, we don't have a mid-term, but let's consider
how to solve the exam scheduling problem. More generally,
exam scheduling is an example of a constraint satisfaction
problem, and we will first learn to solve it using
\emph{backtracking search}.

\section{Modeling Schedules}

If we represent each student by their Spire ID and each course
with its catalog number, we can represent a map from students
to the set of courses that they take with the following
types\footnote{If you're concerned about the runtime cost of allocating new objects, lookup \href{http://docs.scala-lang.org/overviews/core/value-classes.html}{value classes}.}:
\begin{scalacode}
case class Student(spireID: Int)
case class Course(catalogID: Int)
type StudentSchedule = Map[Student, Set[Course]]
\end{scalacode}

The university has several rooms, which we can represent
using their name (e.g., ``CS140''). Moreover, each
room has a finite capacity, which is very relevant for exam
scheduling. We can use the following types to represent
rooms and their capacities:
\begin{scalacode}
case class Room(name: String)
type Capacities = Map[Room, Int]
\end{scalacode}

Let's say that there are $n$ time slots for each exam, so an
exam schedule maps each room and time to a course:
%
\begin{scalacode}
case class Time(n: Int)
type ExamSchedule = Map[(Room, Time), Course]
\end{scalacode}

A schedule is \emph{valid} if every course is given exactly one
exam:
\begin{scalacode}
def isValidExamSchedule(schedule: ExamSchedule, allCourses: List[Course]): Boolean = {
  val scheduleList = schedule.toList
  allCourses.forall { course =>
    scheduleList.filter({ case (loc, c) => c == course }).length == 1
  }
}
\end{scalacode}

Moreover, a schedule is \emph{consistent} if no student has to be in two places
at once:

\begin{scalacode}
def isConsistentExamSchedule(exams: ExamSchedule,
                             students: StudentSchedule): Boolean = {
  val invExams = exams.toList.map(pair => (pair._2, pair._1)).toMap
  def hasDuplicateTimes(alist: List[(Room, Time)]): Boolean = {
    alist.length == alist.toSet.size
  }
  students.map(p => p._2)
    .toList
    .forall(courses => hasDuplicateTimes(courses.map(c => invExams(c))))
}
\end{scalacode}

Therefore, an exam schedule has to be valid and consistent to be acceptable
to all students.

\section{Finding a solution}

We will use what is called a \emph{greedy algorithm} to pick an exam schedule.
We write a recursive function that in each recursive call, assigns a class
to a slot, verifies that the schedule is still consistent and valid and then
recurs. If
...
Regular expressions are equivalent to a class of \emph{state machine} called a \emph{nondeterministic finite automaton} or NFA.  It turns out that NFAs can be drawn as a simple kind of graph, and having this graph in your head helps understand what the regular expression algorithm does.  Here's the NFA for the regular expression \texttt{aa}:
...
Note that we assumed that our input strin (the map) and our regular expression (the maze) both started in the same place.  This means that our regular expression must match our \emph{entire input string}.  In practice, people often write regular expressions that simply search for a \emph{substring}.  In many regular expression implementations, you force the regex to match the entire string by prepending the regex with the \texttt{\^} character and appending it with the \texttt{\$} character.  For example, \texttt{aa} becomes \texttt{\^aa\$}.

Scala's \scalainline{regex.pattern.matcher(str).matches()} (where \scalainline{regex} is a regex and \scalainline{str} is an input string) always tries to match the entire string, so that \texttt{\^} and \texttt{\$} are unnecessary.
