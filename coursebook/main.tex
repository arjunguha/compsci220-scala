...
libraryDependencies += "edu.umass.cs" %% "compsci220" % "1.0.0"
\end{scalacode}
...
\begin{enumerate}
  ...
  \begin{scalacode}
  sealed trait BinTree[A]
  case class Node(lhs: BinTree[A], value: A, rhs: BinTree[A]) extends BinTree[A]
  case class Leaf[A]() extends BinTree[A]	
  \end{scalacode}
  ...
  \begin{instructor}
  Note that this can be done in two ways. The methods could be implemented
  in \scalainline{BinTree} by pattern-matching, or written in an 
  object-oriented style by implementing them in \scalainline{Node}
  and \scalainline{Leaf}.
  \end{instructor}
...
\scalainline{T2}, \scalainline{T3}, and \scalainline{T4} 
...
traits \scalainline{T1}, \scalainline{T2}, \scalainline{T3}, and
\scalainline{T4} as possible. Some classes may be able to extend several
...
class Vector2D(x: Double, y: Double) {
  ...
  def *(factor: Double): Vector2D = {
    new Vector2D(factor * x, factor * y)
  }
...
very cumbersome to use from Scala. Compares to the terse Scala programs
...
very unreadable very quickly. So, you should use them with case.
...
Something that covers everything done so far. Due after Spring Break. Try to make students
