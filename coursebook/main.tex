...
\section{Binary Search Trees\classtime{15}}

\begin{figure}
\begin{scalacode}
sealed trait BinTree
case class Node(lhs: BinTree, key: Int, value: String, rhs: BinTree) extends BinTree
case class Leaf() extends BinTree

def find(k: Int, t: BinTree): String = t match {
  case Leaf() => sys.error("not found")
  case Node(lhs, key, value, rhs) => {
    if (k == key) { 
      value 
    }
    else if (k < key) { 
...
def insert(k: Int, v: String, t: BinTree): BinTree = t match {
  ...
  case Node(lhs, key, value, rhs) => {
    if (k == key) { 
      ...
      Node(insert(k, v, lhs), key, value, rhs) 
    }
    else { 
      Node(lhs, key, value, insert(k, v, rhs)) 
    }
  }
}

def size(t: BinTree): BinTree = t match {
  case Leaf() => 1
  case Node(lhs, k, v, rhs) => size(lhs) + size(rhs) + 1
}
\end{scalacode}
...
keys are integers and values are strings. There are two
kinds of binary trees: (1) empty trees, or \scalainline{Leaf{}}s, and (2)
non-empty trees, or \scalainline{Node(..)}s, which have a left-subtree,
a numertic key, a string value, and a right-subtree. 

Given this definition, we can write canonical functions such
as \scalainline{count}, \scalainline{find}, and \scalainline{insert}.
...
\paragraph{Testing}
There is a lot of code in \cref{bintree} that needs to be tested. There
are two kinds of tests that we can write. 

[FILL] - testing corner cases

[FILL] - property-based testing

\section{Lists}

[FILL] lists using case classees, some list processing functions
...
For your reference, here's \scalainline{map}:
...
For example, given \scalainline{val birds = List(Duck(), Duck(), Goose())}, your program should produce \scalainline{List("dogfood", "dogfood", "pate")}.  Since this is just an in-class exercise, you should not harm any actual ducks or geese.
