...
\begin{figure}
\begin{scalacode}
def incrList(alist: List[Int]): List[Int] = alist match {
  case Nil => Nil
...
\begin{subfigure}{.45\textwidth}
\begin{scalacode}
def concat(alist: List[String]): String = alist match {
  case Nil => ""
...
\begin{subfigure}{.45\textwidth}
\begin{scalacode}
def totalLen(alist: List[String]): Int = alist match {
  case Nil => 0
...
\hline
& \alistinline[language=scala]|val x = 20; def f(x: Int): Int = { x + 5 }; f(10) + x|
& \textrm{Original expression} \\
= & \alistinline[language=scala]|def f(x: Int): Int = { x + 5 }; f(10) + 20|
& \textrm{Substitute \alistinline[language=scala]|x| with \alistinline[language=scala]|20|} \\
= & \alistinline[language=scala]|(10 + 5) + 20|
& \textrm{Inline \alistinline[language=scala]|f| and substitute \alistinline[language=scala]|x| with \alistinline[language=scala]|10|} \\
= & \alistinline[language=scala]|15 + 20|
& \textrm{Evaluate \alistinline[language=scala]|10 + 5|} \\
= & \alistinline[language=scala]|35|
& \textrm{Evaluate \alistinline[language=scala]|15 + 20|}
\end{array}
\]
...
\hline
& \multicolumn{2}{l}
{\alistinline[language=scala]|def makeAdder(x: Int): Int => Int = { def add(y): Int = { x + y }; add }; val add10 = makeAdder(10)|}  \\
= & \alistinline[language=scala]|val add10 = { def add(y): Int = { x + 10 }; add }|
& \textrm{Inline \alistinline[language=scala]|makeAdder| and substitute \alistinline[language=scala]|y| with
\alistinline[language=scala]|10|} \\
= & \alistinline[language=scala]|val add10 = { val add = ((y: Int) => { x + 10 }); add }| & \textrm{Rewrite \alistinline[language=scala]|add| using \alistinline[language=scala]|val| notation} \\
= & \alistinline[language=scala]|val add10 = ((y: Int) => { x + 10 })|
& \textrm{Substitute \alistinline[language=scala]|add| with its definition} \\
= & \alistinline[language=scala]|def add10(y: Int) = x + 10|
& \textrm{Rewrite \alistinline[language=scala]|add| using \alistinline[language=scala]|def| notation}
\end{array}
\]
