...
You are in charge of a pat\'e factory.  One of the annoying facts about making pat\'e is that ducks and geese are friendly, so ducks are always mixing themselves in with the geese.  But you realize that you can make a little extra money for the company if you can build a machine that turns ducks into dogfood.

Spec: Given a \scalainline{List[Bird]}, write a program that uses \scalainline{map} to output \scalainline{"pate"} whenever it encounters a \scalainline{Goose} and \scalainline{"dogfood"} whenever it encounters a \scalainline{Duck}.
...
For example, given \scalainline{val birds = List(Duck, Duck, Goose)}, your program should produce \scalainline{List("dogfood", "dogfood", "pate")}.  As this is just an in-class exercise, you should not harm any actual ducks or geese.

\section{Exercise \#2}

Write a ScalaTest (consult your notes from Lecture \#1) that tests that your function really does convert \scalainline{List(Duck, Duck, Goose)} into \scalainline{List("dogfood", "dogfood", "pate")}.
...
There's a pattern here.  We started with an initial value (\scalainline{sum = 0}) and then we applied a function (\scalainline{+}) over and over again to that value and a new value from our list.  This is the pattern that \scalainline{fold} solves, but it is designed in such a way (using generic types \scalainline{A} and \scalainline{B}) so that you can use it in many situations, not just those involving addition.
...
Your boss likes your idea about turning ducks into dogfood, but he wants to know first how much money you are likely to make in a variety of scenarios.  Pat\'e is fancy, and each can of pat\'e made from a Goose goes for \$10.  Each can of dog food made from a Duck goes for \$1.25.
