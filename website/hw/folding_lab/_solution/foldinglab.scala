...
def product(lst: List[Int]): Int = lst match {
    case Empty() => 1
    case Cons(i, rest) => i * product(rest)
}
...
// Question 2:
// Rewrite the `sum' function (call it `baseSum') that returns a given result
// for empty lists.
def baseSum(onEmpty: Int, lst: List[Int]): Int = lst match {
    case Empty() => onEmpty
    case Cons(i, rest) => i + baseSum(onEmpty, rest)
}
...
// and a list of integers and returns the result of applying the operation to
// all elements of the given list.
def genOp(op: ((Int, Int) => Int), onEmpty: Int, lst: List[Int]): Int = lst match {
    case Empty() => onEmpty
    case Cons(i, rest) => op(i, genOp(op, onEmpty, rest))
}
...
def fold[A](op: ((A,A) => A), onEmpty: A, lst: List[A]): A = lst match {
    case Empty() => onEmpty
    case Cons(i, rest) => op(i, fold(op, onEmpty, rest))
}
...
// Question 5:
// Write the `fold2' function, which takes a function ((A, A) => B), a value for
// empty lists, and a list of the function's input type.
def fold2[A,B](op: ((A,B) => B), onEmpty: B, lst: List[A]): B = lst match {
    case Empty() => onEmpty
    case Cons(i, rest) => op(i, fold2(op, onEmpty, rest))
}
